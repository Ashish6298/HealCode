...
class MarketplaceCommand(BaseCommand):
    ...
    def run(self, args: argparse.Namespace, config: ProjectConfig) -> int:
        if args.marketplace_action == "search":
            ...
            print_success(f"Plugin [bold green]{args.plugin_name}[/] is already at the latest version.")
            return 0

        elif args.marketplace_action == "remove":
            print_success(f"Successfully removed plugin [bold red]{args.plugin_name}[/bold red].")
            return 0
